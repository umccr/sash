...
#### [GRIDSS/GRIPSS](https://github.com/PapenfussLab/gridss)
- `{tumor_id}.gridss.vcf.gz`: VCF containing structural variant calls produced by GRIDSS2.
...
Variants that do not meet these criteria will be filtered out unless they qualify for [Clinical Significance Exceptions](#clinical-significance-exceptions):
...
6. Run PCGR to generate the final report.
...
2. PURPLE:
   - Combines the GRIPSS-filtered SV calls with copy number variation (CNV) data and tumor purity/ploidy estimates.
3. Annotation:
...
- Description: Contains homologous recombination deficiency (HRD) score from DRAGEN analysis (TSV format).
...
A: In Somatic SV, we use SAGE for variant calling, then annotate the variants using PCGR, followed by filtering. Variants with high-tier ranks (TIER_1 or TIER_2) are not filtered out regardless of other criteria.
...
A: The TMB displayed is calculated by PCGR.

### Q: What filtered VCF is the source for the mutational signatures?
A: We use the filtered VCF (after applying quality filters but retaining clinically significant variants) for mutational signatures analysis.
...
### Q: Do the GRIPSS steps do something more than what's happening in Oncoanalyser?
A: No, the same GRIPSS parameters are applied, with the only difference being the reference files used.
